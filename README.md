...
### 3. ✨ 温暖启航寄语与动态轮播 (Motivational Quotes in Translation Frame)
* **主翻译框原生默认轮播**：在应用启动后、用户尚未开始打字时，卡片核心翻译展区将直接**随机动态轮播**激励寄语及其纯正英译与重点词汇（无需多余公告栏，极致简洁美观）：
  * *“开始学习吧！”* · *“今天也要努力上哦！”*
  * *“你说过的，要改变世界”* · *“关注B站的北辰捡垃圾喵”*
  * *“我相信你只要,做什么都可以”* · *“勇敢去做”*
* **敲字即隐机制**：一旦监测到用户在任意软件中开始敲字输入，**寄语轮播将瞬时永久隐匿且不再显示**，无缝切换为专注呈现用户的输入翻译与词汇解析，极致纯净无打扰。

---

## 💡 为什么需要它？ / Why English Shadow Companion?

| 传统外贸/翻译软件 / Traditional Tools | 英语影子伴侣 / English Shadow Companion |
| :--- | :--- |
...
* **提炼核心表达**：自动从整句英文中萃取 1~3 个关键生词与高频地道短语；
* **单词独立点读**：每个单词都是独立的 Apple 风格胶囊微标签，**鼠标点击任意标签，即可单独听取该生词的标准发音**。

### 4. 智能回车截流感知 (Smart Enter & Debounce Engine)
* **微信回车秒抓**：针对聊天软件按回车即清空输入框的特性，底层精确拦截 <kbd>Enter</kbd> 触发点，**保证刚发送出去的句子绝不丢失**；
* **严格输入框识别**：严格限定仅在真正的文本输入区域提取文字，绝不误抓桌面图标、窗口标题或按钮文字。
...
| **单项点读** / Word Audio | 直接点击生词小标签 | 单独朗读选中的重点单词或词组 |
| **复制英文** / Copy | 点击 `[复制]` 按钮 | 快速复制英文到剪贴板，带绿色状态反馈 |
| **一键即时学** / Learn Now | 按 <kbd>F8</kbd> 或连按两次 <kbd>Ctrl</kbd> | 强制学习当前光标所在输入框或剪贴板内容 |
| **退出伴侣** / Quit | 点击红色红绿灯 `×` | 安全退出后台守护进程 |
...
### 安装与运行 / Installation & Launch

```bash
# 1. 克隆代码仓库 / Clone Repository
git clone https://github.com/opopile/EnglishShadowCompanion.git
cd EnglishShadowCompanion

# 2. 安装 Python 依赖 / Install Dependencies
pip install -r requirements.txt

# 3. 启动伴侣 / Launch Application
python main.py
```

> **提示**：双击根目录下的 `run.bat` 即可一键直接启动；若需完全无控制台黑框后台静默运行，双击 `launch.vbs` 即可。
...
本项目遵循 [MIT License](LICENSE) 开源协议，欢迎自由使用、定制与分享！
